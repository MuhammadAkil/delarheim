...
const CarListCarousel: React.FC<PropType> = (props) => {
        ...
        <div className="min-w-full rounded-lg cursor-pointer relative group border shadow-lg md:min-w-[50%] lg:min-w-[33.3333%] flex justify-center">
          <div className="bg-white rounded-lg w-full">
            ...
            <div className="absolute z-50 inset-0 h-[54%] rounded-2  flex justify-center items-center bg-black bg-opacity-30 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease ">
              <span className="bg-[#6b5fff] p-3 cursor-pointer rounded-lg">
                View Details
              </span>
            </div>
          </div>
  ...
  <div className="absolute top-1/2 -left-10 -right-10 z-10 flex justify-between items-center -translate-y-1/2 px-5">
    <PrevButton
      ...
      fillColor="#ffffff"
      className="border w-14 text-white !bg-[#6b5fff] rounded-full"
    />
    <NextButton
      onClick={onNextButtonClick}
      disabled={nextBtnDisabled}
      fillColor="#ffffff"
      className="border w-8 h-8 text-white !bg-[#6b5fff] rounded-full"
    />
  </div>
</div>
